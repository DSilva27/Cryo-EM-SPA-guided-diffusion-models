...
class GuidedAtomDiffusion(AtomDiffusion):
    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        guidance_model = parse_guidance_yaml(steering_args["guidance_config"])
        if isinstance(guidance_model, PointCloudGuidanceModel):
            assert (
                guidance_model.target_point_clouds.shape[0] == multiplicity
            ), "Not enough guidance point clouds for the requested multiplicity."
        # torch.save(atom_mask, "atom_mask.pt")
        ...
        for step_idx, (sigma_tm, sigma_t, gamma) in enumerate(sigmas_and_gammas):
            ...
            for i in range(multiplicity):
                writers[i].write(coords_for_writer[i])
...
class Boltz1Guided(LightningModule):
    ...
    def validation_step(self, batch: dict[str, Tensor], batch_idx: int):
        ...
        try:
            ...
            if "out of memory" in str(e):
                ...
                raise e
        # if the multiplicity used is > 1
        # then we take the best lddt of the different samples
        ...
        if self.confidence_prediction and n_samples > 1:
            # note: for now we don't have pae predictions
            # so have to use pLDDT instead of pTM
            # also, while AF3 differentiates the best prediction per confidence
            # type we are currently not doing it
            # consider this in the future as well as weighing the different pLLDT
            # types before aggregation
