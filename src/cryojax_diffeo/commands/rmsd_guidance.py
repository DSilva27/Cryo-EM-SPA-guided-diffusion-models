...
# Boltz Imports
from boltz.data.types import Manifest, MSA, Record
...
def process_inputs(
    ...
    if has_basic_auth and has_api_key:
        raise ValueError(
            "Cannot use both basic authentication "
            "(--msa_server_username/--msa_server_password) "
            "and API key authentication (--api_key_header/--api_key_value). "
...
@click.option(
    "--msa_server_username",
    type=str,
    help="MSA server username for basic auth."
    + " Used only if --use_msa_server is set. "
    + "Can also be set via BOLTZ_MSA_USERNAME environment variable.",
    default=None,
)
@click.option(
    "--msa_server_password",
    type=str,
    help="MSA server password for basic auth. "
    + " Used only if --use_msa_server is set. "
    + "Can also be set via BOLTZ_MSA_PASSWORD environment variable.",
    default=None,
)
@click.option(
    "--api_key_header",
